...
| r2-gcp (wg1) | `34.97.197.104` | GCP 予約 static IP | VyOS 静的経路 + `dhcp-interface eth1` |

#### r2-gcp (wg1) — 静的な /32 escape

GCP 側の公開 IP は固定なので VyOS config 内で完結できる。next-hop には `dhcp-interface eth1` を使うことで、自宅検証環境 / venue 本番いずれでも eth1 の DHCP default GW が自動選択される。

```
set protocols static route 34.97.197.104/32 dhcp-interface eth1
```

投入前は `traceroute 34.97.197.104` が `via 10.255.0.1 dev wg0` (BGP default に吸収)、投入後は `via <DHCP GW> dev eth1` になる。これで wg1 の外殻パケットが wg0 の中にカプセル化される二重トンネル化が防げる。
