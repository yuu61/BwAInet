...
VyOS 内蔵 PowerDNS Recursor をフルリゾルバ（再帰解決）として使用。全 VLAN および VyOS 自身からのクエリを受け付ける。上流フォワーダーは指定せず、ルートから再帰解決する。`dns forwarding system` は `system name-server` が自分自身の場合にフォワーディングループを起こすため使用しない（r1 と同様）。クエリログは法執行機関対応のため有効化する。DNSSEC は `process-no-validate` とし、DNSSEC 関連 RR は処理するが検証失敗で利用者トラフィックを止めない。
...
# DNSSEC は処理のみ。validation failure で利用者通信を止めない
...
> `source-address` は設定しない。2026-04-13 の検証で `source-address 2001:ce8:180:5a79::1` を明示すると PowerDNS Recursor が IPv4 の外向き再帰問い合わせを無効化し、`twitter.com` / `x.com` / `t.co` など一部 zone が `SERVFAIL` になった。フルリゾルバとして運用する場合は送信元アドレス選択をカーネルに任せ、IPv4/IPv6 の両 transport を有効に保つ。
...
# WireGuard 直接: local-pref 250 (優先)
...
高 BDP 経路 (自宅 r1 との 10G 直結 + WireGuard トンネル) で帯域を引き出すため、以下の sysctl を永続適用している。BBR は輻輳制御、fq は pacing で協調動作する。TCP バッファは 1GbE 時代のカーネルデフォルトから 128 倍に拡張し、0.5-2 Gbps クラスの BDP に追従できるよう設定。
...
# === UDP バッファ (WireGuard の underlying UDP) ===
...
- **BBR + fq**: BBR は送信レート推定で cwnd を制御するため fq qdisc の packet pacing と組み合わせが前提。`net.core.default_qdisc=fq` は新規作成インターフェースに適用されるデフォルト qdisc
- **tcp_rmem/wmem max = 16 MiB**: 1 Gbps × 100 ms RTT = 12.5 MiB BDP をカバー。WG トンネル経由の RTT 増を加味しても余裕
- **udp_mem 上限 16 MiB**: WireGuard の encrypted UDP 送受信で一時的なバースト吸収用
- **netdev_max_backlog = 5000**: per-CPU RX バックログキュー長。IRQ バースト時の drop 耐性向上

#### Proxmox 側との対応関係

これらの sysctl は VyOS VM 内の TCP/UDP 動作に影響する。Proxmox host 側では nic3 (WAN) の ring buffer 4096 と ntuple off が永続適用されており ([`venue-proxmox.md`](venue-proxmox.md#nic3-wan-の-i40e-パフォーマンスチューニング) 参照)、両者で協調して高スループットを実現している。

## 15. 完全コンフィグ一覧

投入用の VyOS CLI コマンド集は [`../configs/r3-venue.conf`](../configs/r3-venue.conf) を参照。本設計書の各セクションに記載した snippet と完全に同期している。
