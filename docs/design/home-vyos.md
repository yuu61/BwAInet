...
実装: `/etc/systemd/system/conntrack-logger.service` + `/usr/local/sbin/r1-conntrack-logger.sh` (リポジトリ内 `scripts/r1-home/conntrack-logger.sh` と `scripts/vyos-common/conntrack-logger.service`)。VyOS 再起動後も自動起動。
...
- スクリプト: [`../../scripts/r1-home/pd-update-venue.sh`](../../scripts/r1-home/pd-update-venue.sh)
...
r1 の設定 API。管理 PC から `scripts/r1-home/config.py` (curl ベース) で設定投入する。
